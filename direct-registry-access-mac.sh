#!/bin/bash
# Script to directly read and display images from the registry (macOS compatible)
...
find "$REGISTRY_DIR" -name "*.json" | while read -r file; do
  ...
  # Get creation time (macOS compatible)
  CREATED=$(stat -f "%Sm" -t "%Y-%m-%d" "$file")
