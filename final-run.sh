...
echo "🚀 Starting Final Fixed SentinelStacks Enhanced Application..."
...
# Add the additional stylesheet to the HTML if not already present
if ! grep -q "additional-styles.css" "web-ui/index.html"; then
  echo "📄 Adding additional stylesheet to index.html..."
  sed -i '' 's/<\/head>/<link rel="stylesheet" href="\/additional-styles.css" \/><\/head>/' web-ui/index.html
fi
